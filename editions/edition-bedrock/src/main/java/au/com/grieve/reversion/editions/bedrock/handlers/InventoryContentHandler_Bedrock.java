...
public class InventoryContentHandler_Bedrock extends PacketHandler<BedrockTranslator, InventoryContentPacket> {
    public static final InventoryContentHandler_Bedrock INSTANCE = new InventoryContentHandler_Bedrock();

    @Override
    public boolean fromDownstream(BedrockTranslator translator, InventoryContentPacket packet) {
        packet.setContents(
                Arrays.stream(packet.getContents())
                        .map(i -> translator.getItemMapper().mapItemDataToUpstream(i))
