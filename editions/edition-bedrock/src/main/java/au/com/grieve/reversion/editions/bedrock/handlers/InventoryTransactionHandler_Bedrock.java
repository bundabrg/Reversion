...
public class InventoryTransactionHandler_Bedrock extends PacketHandler<BedrockTranslator, InventoryTransactionPacket> {
    public static final InventoryTransactionHandler_Bedrock INSTANCE = new InventoryTransactionHandler_Bedrock();

    @Override
    public boolean fromUpstream(BedrockTranslator translator, InventoryTransactionPacket packet) {
        packet.setItemInHand(translator.getItemMapper().mapItemDataToDownstream(packet.getItemInHand()));
        ...
        for (InventoryActionData actionData : originalActions) {
            InventoryActionData translated = new InventoryActionData(
                    ...
                    actionData.getSlot(),
                    translator.getItemMapper().mapItemDataToDownstream(actionData.getFromItem()),
                    translator.getItemMapper().mapItemDataToDownstream(actionData.getToItem()),
